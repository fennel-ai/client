...
def test_PyArrowSchemaConversion():
    ...
    assert get_pyarrow_field('x', Optional[int]) == pa.field('x',
    ...
    assert get_pyarrow_field('x', Union[int, str]) == pa.union([pa.field('x',
        pa.int64(), nullable=False), pa.field('x', pa.string(),
        nullable=False)], mode='dense', )
    ...
    assert get_pyarrow_schema(Tuple[int, str]) == pa.struct(
        [pa.field('f0', pa.int64(), nullable=False), pa.field('f1', pa.string(
        ), nullable=False)])
    assert get_pyarrow_schema(List[int]) == pa.list_(pa.int64())
    assert get_pyarrow_schema(List[int]) == pa.list_(pa.int64())
    assert get_pyarrow_schema(Set[int]) == pa.map_(pa.int64(),
        pa.null())
    assert get_pyarrow_schema(Dict[int, int]) == pa.map_(pa.int64(),
        pa.int64())
    assert get_pyarrow_schema(Dict[str, float]) == pa.map_(
        pa.string(),
        pa.float64())
    assert get_pyarrow_schema(Dict[str, Dict[int, List[float]]]) == \
           pa.map_(
               pa.string(),
               pa.map_(pa.int64(), pa.list_(pa.float64())))
    ...
    assert get_pyarrow_schema(Tuple[List[Dict[str, float]], Dict[str, List[Dict[
        int, int]]]]) == \
           pa.struct(
               [pa.field('f0', pa.list_(pa.map_(pa.string(),
                   pa.float64())), nullable=False),
                pa.field('f1', pa.map_(pa.string(), pa.list_(
                    pa.map_(pa.int64(), pa.int64(), ))), nullable=False)])
