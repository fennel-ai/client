...
def test_InvalidDType():
    ...
    with pytest.raises(TypeError) as e:
        validate_schema(Schema([Field("actor_id", dtype=Map(), default=1),
                                Field("timestamp", dtype=Int, default=0)],
        ))
    assert str(e.value) == "invalid key type in map: None"
    with pytest.raises(TypeError) as e:
        validate_schema(Schema([Field("actor_id", dtype=Map(String), default=1),
                                Field("timestamp", dtype=Int, default=0)],
        ))
    assert str(e.value) == "invalid value type in map: None"
    with pytest.raises(TypeError) as e:
        validate_schema(
            Schema([Field("actor_id", dtype=Map(String, Array()), default=1),
                    Field("timestamp", dtype=Int, default=0)],
            ))
    assert str(e.value) == "invalid array type: None"
...
def test_InvalidDefaultValue():
    with pytest.raises(SchemaException) as e:
        validate_schema(Schema([Field("actor_id", dtype=Int, default=1.0),
                                Field("actor_name", dtype=Int,
                                    default="string")]
        ))
    assert (
    ...
    with pytest.raises(SchemaException) as e:
        validate_schema(Schema([Field("actor_id", dtype=String, default=1)], ))
    assert (
            str(e.value) == "Schema errors: [TypeError('Expected default value for "
                            "field actor_id to be str, got 1')]")
    with pytest.raises(SchemaException) as e:
        validate_schema(
            Schema([Field("actor_id", dtype=Array(String), default=1)], ))
    assert (
            str(e.value) == "Schema errors: [TypeError('Expected default value for "
