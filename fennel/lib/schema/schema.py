...
def get_pyarrow_field(name: str, type_: Any) -> pa.lib.Field:
    ...
    if get_origin(type_) is Union and type(None) == get_args(type_)[1]:
        return pa.field(
            name, get_pyarrow_schema(get_args(type_)[0]), nullable=True
        )
    elif get_origin(type_) is Union:
        x = [get_pyarrow_field(name, t) for t in get_args(type_)]
    ...
    if type_ is None:
        ...
        return pa.struct(
            [
                ...
                for i, t in enumerate(get_args(type_))
            ]
        )
    elif get_origin(type_) is list:
        return pa.list_(get_pyarrow_schema(get_args(type_)[0]))
    elif get_origin(type_) is set:
        return pa.map_(get_pyarrow_schema(get_args(type_)[0]), pa.null())
    elif get_origin(type_) is dict:
        return pa.map_(
            get_pyarrow_schema(get_args(type_)[0]),
