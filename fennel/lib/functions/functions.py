...
from fennel.dtypes import Window, Sliding, Tumbling
from fennel.internal_lib.duration import Duration, duration_to_timedelta


def bucketize(
    timestamp: Union[datetime, pd.Timestamp],
    window: Union[Duration, Tuple[Duration, Duration], Sliding, Tumbling],
) -> pd.Series:
    """
    ...
    :param window: Can be four types:
        ...
        3. If Sliding is passed like Sliding("1d", "1h") then considers a sliding window of duration 1d with stride 1hr.
