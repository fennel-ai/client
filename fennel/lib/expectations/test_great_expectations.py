...
import fennel.lib.expectations as exp
...
def test_dataset_expectation_creation(grpc_stub):
    ...
    d = {
        ...
        "metadata": {"owner": "test@test.ai"},
    }
    expected_exp_request = ParseDict(d, exp_proto.Expectations())
    act_config = sync_request.dataset_requests[0].expectations
...
def test_dataset_invalid_expectation_creation():
    with pytest.raises(Exception) as e:
        ...
        class UserInfoDSInvalid:
            ...
            def dataset_expectations(cls):
                return [
                    exp.expect_column_values_to_be_between_random(
                        column=str(cls.age), min_value=0, max_value=100
                    ),
                    exp.expect_column_values_to_be_in_set(
                        column=str(cls.gender),
    ...
    assert (
        str(e.value)
        == "module 'fennel.lib.expectations' has no attribute 'expect_column_values_to_be_between_random'"
    )
