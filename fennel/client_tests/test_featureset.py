...
class UserInfoDataset:
    ...
    def get_expectations(cls):
        return [expect_column_values_to_be_between(column="age", min_value=1, max_value=100, mostly=0.95)]
...
class TestSimpleExtractor(unittest.TestCase):
    ...
    def test_get_age_and_name_features(self):
        ...
        df = UserInfoMultipleExtractor.get_age_and_name_features(UserInfoMultipleExtractor.original_cls, ts, age, name)
    ...
    def test_simple_extractor(self, client):
        ...
        df = UserInfoSingleExtractor.get_user_info(UserInfoMultipleExtractor, ts, user_ids)
        self.assertEqual(df.shape, (2, 4))
        self.assertEqual(df["UserInfoSingleExtractor.age"].tolist(), [32, 24])
        self.assertEqual(df["UserInfoSingleExtractor.age_squared"].tolist(), [1024, 576])
        self.assertEqual(df["UserInfoSingleExtractor.age_cubed"].tolist(), [32768, 13824])
        self.assertEqual(df["UserInfoSingleExtractor.is_name_common"].tolist(), [True, False])

        series = UserInfoMultipleExtractor.get_country_geoid_extractor(UserInfoMultipleExtractor, ts, user_ids)
...
class TestExtractorDAGResolutionComplex(unittest.TestCase):
    ...
    def test_dag_resolution_complex(self, client):
        ...
        self.assertEqual(
            feature_df["UserInfoTransformedFeatures.country_geoid_square"].tolist(),
            [25, 9],
        )
...
class DocumentFeatures:
    ...
    def get_doc_features(cls, ts: pd.Series, doc_id: pd.Series):
        ...
        df[str(cls.bert_embedding)] = df[str(cls.bert_embedding)].apply(lambda x: x if x is not None else [0, 0, 0, 0])
        df[str(cls.fast_text_embedding)] = df[str(cls.fast_text_embedding)].apply(
...
class TestDocumentDataset(unittest.TestCase):
    ...
    def test_document_featureset(self, client):
        client.sync(datasets=[DocumentContentDataset], featuresets=[DocumentFeatures])
        now = datetime.utcnow()
        data = [
        ...
        feature_df = client.extract_features(
            ...
            input_dataframe=pd.DataFrame({"DocumentFeatures.doc_id": [18232, 18234]}),
        )
