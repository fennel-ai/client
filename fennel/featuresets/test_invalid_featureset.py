...
class UserInfo2:
    ...
    home_geoid: int = feature(id=2)
    age: int = feature(id=3).extract(field=UserInfoDataset.age, default=0)
...
def test_invalid_autogenerated_extractors(client):
    ...
    assert (
        str(e.value)
        == "Feature `UserInfo.age` has type `<class 'int'>` but expectected type `Optiona[<class 'int'>]`."
    )
    ...
    assert (
        str(e.value)
        == "Feature `UserInfo.age` has type `<class 'float'>` but expected type `<class 'int'>`."
    )
    ...
    assert (
        str(e.value)
        == "Feature `UserInfo.home_geoid` has type `<class 'float'>` but the extractor aliasing `UserInfo2.home_geoid` has input type `<class 'int'>`."
    )
    ...
    assert (
        str(e.value)
        == "Default value `0.0` for feature `UserInfo.age` has incorrect default value: Value `0.0` does not match type `int`"
    )
