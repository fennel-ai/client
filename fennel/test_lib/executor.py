...
class Executor(Visitor):
    ...
    def execute(self, pipeline: Pipeline, dataset: Dataset) -> Optional[NodeRet]:
    ...
    def visitDataset(self, obj) -> Optional[NodeRet]:
        if obj._name not in self.data:
            return None
        return NodeRet(self.data[obj._name], obj.timestamp_field, obj.key_fields)
    ...
    def visitTransform(self, obj) -> Optional[NodeRet]:
        ...
        try:
            ...
            raise Exception(f"Error in transform function for pipeline " f"{self.cur_pipeline_name}, {e}")
        num_rows = t_df.shape[0]
        if t_df is None:
            raise Exception(f"Transform function {obj.func.__name__} returned " f"None")
        ...
        if not set_match(input_column_names, output_column_names):
            if obj.new_schema is None:
                ...
                if not set_match(output_expected_column_names, output_column_names):
                    raise ValueError(
        ...
        return NodeRet(sorted_df, input_ret.timestamp_field, input_ret.key_fields)
    ...
    def visitFilter(self, obj) -> Optional[NodeRet]:
        ...
        gen_pycode = self.serializer.wrap_function(filter_func_pycode, is_filter=True)
    ...
    def _merge_df(self, df1: pd.DataFrame, df2: pd.DataFrame, ts: str) -> pd.DataFrame:
    ...
    def visitAggregate(self, obj):
        ...
        if len(input_ret.key_fields) > 0:
            ...
            df = df.groupby(input_ret.key_fields).apply(lambda x: x.sort_values(input_ret.timestamp_field).iloc[-1])
        ...
        if self.cur_ds_name in self.agg_state:
            ...
            df = self._merge_df(self.agg_state[self.cur_ds_name], df, input_ret.timestamp_field)
        ...
        return NodeRet(pd.DataFrame(), input_ret.timestamp_field, obj.keys, result, True)
    ...
    def visitJoin(self, obj) -> Optional[NodeRet]:
        ...
        def add_within_high(row):
            return row[left_timestamp_field] + duration_to_timedelta(obj.within[1])

        left_df[ts_query_field] = left_df.apply(lambda row: add_within_high(row), axis=1)
        ...
        def sub_within_low(row):
            if obj.within[0] == "forever":
                ...
                return row[left_timestamp_field] - duration_to_timedelta(obj.within[0])

        left_df[tmp_ts_low] = left_df.apply(lambda row: sub_within_low(row), axis=1)
        ...
        if obj.on is not None and len(obj.on) > 0:
            if not is_subset(obj.on, left_df.columns):
                raise Exception(f"Join on fields {obj.on} not present in left dataframe")
            if not is_subset(obj.on, right_df.columns):
                raise Exception(f"Join on fields {obj.on} not present in right dataframe")
            merged_df = pd.merge_asof(left=left_df, right=right_df, on=ts_query_field, by=obj.on)
        else:
            if not is_subset(obj.left_on, left_df.columns):
                raise Exception(f"Join keys {obj.left_on} not present in left dataframe")
            if not is_subset(obj.right_on, right_df.columns):
                raise Exception(f"Join keys {obj.right_on} not present in right dataframe")
        ...
        merged_df = merged_df.transform(lambda row: filter_bounded_row(row), axis=1)
        ...
        for index, dtype in zip(original_dtypes.index, original_dtypes.values):
            if index in right_df.columns.values and index not in left_df.columns.values:
        ...
        merged_df[left_timestamp_field] = merged_df.apply(lambda row: emited_ts(row), axis=1)
    ...
    def visitRename(self, obj):
        ...
        if not is_subset(obj.column_mapping.keys(), df.columns):
            raise Exception(f"Columns to be renamed {obj.column_mapping.keys()} not present in dataframe")
        # Check that the columns post renaming are not present in the dataframe
        if is_subset(obj.column_mapping.values(), df.columns):
            raise Exception(f"Columns after renaming {obj.column_mapping.values()} already present in dataframe")
