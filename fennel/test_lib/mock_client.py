...
class _DatasetInfo:
    fields: List[str]
    key_fields: List[str]
    erase_key_fields: List[str]
    timestamp_field: str
    is_source_dataset: bool
    on_demand: OnDemand
    erased_key: List[Dict[str, Any]]
...
class MockClient(Client):
    ...
    def erase(self, dataset_name: str, erase_keys: List[Dict[str, Any]]):
        ...
        for erase_key in erase_keys:
            for field_name in erase_key.keys():
                if field_name not in dataset_info.fields:
                    raise ValueError(f"Field: {field_name} not in dataset")
                if field_name not in dataset_info.key_fields:
                    raise ValueError(
                        f"Field: {field_name} not in a key field in dataset"
                    )
                if field_name not in dataset_info.erase_key_fields:
                    raise ValueError(
                        f"Field: {field_name} is not an erase key field in dataset"
                    )
            if len(erase_key.keys()) != len(dataset_info.erase_key_fields):
                raise ValueError("Not enough erase key were provided")

        self.dataset_info[dataset_name].erased_key.extend(erase_keys)

        def is_kept(row):
            for erase_key in self.dataset_info[dataset_name].erased_key:
                match_all = True
        ...
        self.data[dataset_name] = data[data.apply(is_kept, axis=1)]
        return FakeResponse(200, "OK")
    ...
    def _merge_df(self, df: pd.DataFrame, dataset_name: str):
        ...
        self.data[dataset_name] = self.data[dataset_name][
            self.data[dataset_name].apply(is_kept, axis=1)
        ]
