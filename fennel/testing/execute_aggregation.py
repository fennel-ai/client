...
class ExpDecayAggState(AggState):
    def __init__(self, max_exponent, half_life):
        ...
        print("half_life_seconds", half_life_seconds)
        self.decay = math.log(2)/half_life_seconds
        self.log_sum_pos = 0
        self.log_sum_neg = 0
    ...
    def add_val_to_state(self, val, ts):
        ...
        ts = int(ts.timestamp())
        if val >=0:
            self.log_sum_pos += val * math.exp(ts * self.decay - self.max_exponent)
        else:
            self.log_sum_pos += -1 * val * math.exp(ts * self.decay - self.max_exponent)
        return self.get_val()

    def del_val_from_state(self, val, ts):
        ts = int(ts.timestamp())
        if val >=0:
            self.log_sum_pos -= val * (ts * self.decay - self.max_exponent).exp()
        else:
            self.log_sum_pos -= -1 * val * (ts * self.decay - self.max_exponent).exp()
        return self.get_val()
    ...
    def get_val(self):
        ...
        :return:
        """
        return [self.log_sum_pos, self.log_sum_neg, self.max_exponent, self.decay]
...
def get_aggregated_df(
    ...
    if isinstance(aggregate, ExpDecaySum):
        max_exponent = max(df[ts_field])
        max_exponent = int(max_exponent.timestamp())
        print("max_exponent", max_exponent)
    else:
        max_exponent = None
        ...
        if row[FENNEL_ROW_TYPE] == -1:
            ...
            if key not in state:
                if isinstance(aggregate, Sum):
                    ...
                    state[key] = ExpDecayAggState(max_exponent, aggregate.half_life )
                else:
                    raise Exception(
