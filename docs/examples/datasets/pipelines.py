...
from fennel.datasets import Count, Sum, LastK
...
def test_fraud(client):
    # # Sync the dataset
    client.commit(datasets=[Activity, FraudActivityDataset])
    now = datetime.now()
    ...
    assert client.get_dataset_df("FraudActivityDataset").shape == (6, 4)
