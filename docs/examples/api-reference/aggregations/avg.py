    ...
    class Aggregated:
        ...
        def pipeline(cls, ds: Dataset):
            return ds.groupby("uid").aggregate(
                Average(of="amt", window="1d", default=-1.0, into_field="avg_1d"),
                Average(of="amt", window="1w", default=-1.0, into_field="avg_1w"),
            )
    # /docsnip
...
def test_invalid_type(client):
    with pytest.raises(Exception):
        ...
        class Aggregated:
            ...
            def pipeline(cls, ds: Dataset):
                return ds.groupby("uid").aggregate(
                    Average(of="zip", window="1d", default="avg", into_field="max"),
                )
        # /docsnip
...
def test_non_matching_types(client):
    with pytest.raises(Exception):
        ...
        class Aggregated:
            ...
            def pipeline(cls, ds: Dataset):
                return ds.groupby("uid").aggregate(
                    Average(of="amt", window="1d", default=1., into_field="ret"),
                )
        # /docsnip