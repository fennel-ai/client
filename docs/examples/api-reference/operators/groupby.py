...
class TestGroupbySnips(unittest.TestCase):
    ...
    def test_basic(self, client):
        ...
        client.log("webhook", "Transaction", pd.DataFrame([
            {"uid": 1, "category": "grocery", "timestamp": "2021-01-01T00:00:00"},
            {"uid": 2, "category": "electronics", "timestamp": "2021-02-01T00:00:00"},
            {"uid": 3, "category": "grocery", "timestamp": "2021-03-01T00:00:00"},
            {"uid": 3, "category": "electronics", "timestamp": "2021-04-01T00:00:00"},
        ]))
        df = client.get_dataset_df("FirstInCategory")
        assert(df["category"].tolist() == ["grocery", "electronics"])
        assert(df["uid"].tolist() == [1, 2])
        assert(df["timestamp"].tolist() == [datetime(2021, 1, 1, 0, 0, 0), datetime(2021, 2, 1, 0, 0, 0)])
