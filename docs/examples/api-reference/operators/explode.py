    ...
    def test_basic(self, client):
        ...
        class Derived:
            ...
            def pipeline(cls, ds: Dataset):
                return ds.explode("skus", "prices").rename({"skus": "sku", "prices": "price"})
        # /docsnip

        client.sync(datasets=[Orders, Derived])
        # log some rows to the transaction dataset
        client.log("webhook", "Orders", pd.DataFrame([
            {"uid": 1, "skus": [1, 2], "prices": [10.1, 20.0], "timestamp": "2021-01-01T00:00:00"},
            {"uid": 2, "skus": [], "prices": [], "timestamp": "2021-01-01T00:00:00"}
        ]))
        ...
        assert(df["timestamp"].tolist() == [datetime(2021, 1, 1, 0, 0, 0), datetime(2021, 1, 1, 0, 0, 0), datetime(2021, 1, 1, 0, 0, 0)])
